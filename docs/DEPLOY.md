...
| ◑ PARTIAL (F087) | **Full CSP (script-src) — Report-Only** | `src/middleware.ts` ships an app-wide nonce-based CSP as **`Content-Security-Policy-Report-Only`** (prod branch: `script-src 'self' 'nonce-<n>' 'strict-dynamic' https://js.tosspayments.com` — **no `unsafe-inline` for scripts**; dev branch: `unsafe-inline`/`unsafe-eval`/`ws:` for HMR). Next 15.5 auto-nonces its own inline hydration scripts (reads the request CSP header). Violations POST to `/api/csp-report` (`report-uri`/`report-to`; redacted log, 204, rate-limited). **Report-Only chosen because the hermetic Playwright suite mocks the Toss SDK and ABORTS `js.tosspayments.com` (`tests/e2e/_helpers/tossMock.ts`) — it can NEVER prove the real `*.tosspayments.com` allowlist; only a live browser can.** <br>**Enforce flip (deploy-gated, do NOT flip on green CI):** ① Deploy to a Vercel **preview** with real Toss **TEST/sandbox** keys, Report-Only live. ② In a **real browser**, walk all three pay surfaces — 결제위젯 (`CheckoutView`) + 맞춤 written 결제창 (`WrittenForm`) + **≥1 간편결제** (네이버페이/카카오페이/토스페이) — watching `/api/csp-report` + devtools for `script-src`/`connect-src`/`frame-src`/`img-src`/`worker-src` violations. ③ Add any observed missing hosts to `src/lib/csp.ts` (likely `static.toss.im` + 간편결제 logo CDNs e.g. `*.pstatic.net`/`*.kakaocdn.net`; `worker-src` is already `'self' blob:`). ④ **Measure `/`, `/anniversary`, `/first-moments` p95 on the preview** — the per-request nonce disables Next static/ISR caching (pages render dynamically), so confirm the < 2s budget still holds before enforcing. ⑤ Set `REPORT_ONLY = false` in `src/middleware.ts` **and** remove the `{ key: "Content-Security-Policy", value: "frame-ancestors 'none'" }` entry from `next.config.ts` SECURITY_HEADERS (the middleware CSP already carries `frame-ancestors 'none'`) so the browser doesn't get two enforcing CSP headers. Keep `X-Frame-Options: DENY`. | `src/middleware.ts`; `src/lib/csp.ts`; `src/app/api/csp-report/route.ts`; `next.config.ts` |
