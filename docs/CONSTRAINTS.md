...
- Architecture/safety rules → `scripts/check-constraints.mjs` (`pnpm constraints`):
  - **R1** no committed Stripe live keys
...
- **Stripe test mode** in dev; live actions via the approval gate (`docs/SAFETY.md`).
- **Minimize PII.** Collect only what an order needs; never log it (use `redact()`).
- **Prices in integer cents**; capture price at purchase time on `OrderItem`.
