...
### Active-template revalidation (FIXED post-impl-review, hermetic-safe)
`getTemplateByKey` rejects unknown keys (→400) AND — per the worker≠checker review — now **rejects a deactivated DB row** (`row.active === false → null`), mirroring `getTemplatesByCategory`'s `active:true` filter, so a withdrawn product cannot be ordered in production. Implemented as a minimal `TemplateRow.active?` + one-line guard in the merged, no-concurrent-writer `templates.ts` (a conflict-free scope deviation, precedent ADR-0011 which also edited `templates.ts`). Hermetic mode is unaffected (the seed mirror has no inactive rows and is unchanged), so the change is gate-safe; the DB-branch guard is production-path (gate-exercised only in production, like its sibling DB branches — PROGRESS follow-up #1). This makes the "revalidate active" line above true rather than aspirational.

### Production checkout is an explicit, gated seam (FIXED post-impl-review)
The real Toss browser-SDK flow is **not wired** in this build. `/api/payments/create` therefore returns the sandbox `payUrl` only outside production, and **fails fast with an honest 503** ("결제 기능이 아직 준비되지 않았습니다.") when `APP_ENV === "production"` — never a silent broken redirect to the prod-`notFound()` pay page. The production wiring is the documented seam below.
