...
// F081 — 입금대기(가상계좌) 운영. 관리자 상세에 발급 계좌·기한(KST)·만료 여부가 보이고, 미입금 종료는
...
test.describe("admin 입금대기(VA) 운영 (F081)", () => {
  test("만료 주문: VA 정보 렌더 → 잘못된 토큰 거부 → 대상 바인딩 토큰으로 종료 → 감사 기록", async ({ page }) => {
    const orderId = await completeVirtualAccountOrder(page, { email: "f081expired@example.com", expired: true });

    // 스펙별 고유 관리자(plus-address 폴백 패밀리) — 병렬 스펙 간 OTP 단일사용/발송캡 경합 방지
    await loginAs(page, "admin+f081va@example.com");
  ...
  test("기한 전 주문: 올바른 토큰이라도 종료가 서버에서 거부된다 (무게이트 취소 봉쇄)", async ({ page }) => {
    const orderId = await completeVirtualAccountOrder(page, { email: "f081active@example.com" });

    await loginAs(page, "admin+f081va2@example.com");
    await page.goto(`/admin/orders/${orderId}`);
