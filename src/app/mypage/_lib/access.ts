 ...
 * with a deterministic dev fallback so the hermetic verify/E2E flow works. That fallback is now
 * gated by the F074 dev-auth opt-in (`devAuthEnabled`: non-prod + ALLOW_DEV_AUTH) — this key also
 * signs the account session (session.ts) and OTP hashes (otp.ts), so a public preview/staging box
 * WITHOUT the flag must FAIL-CLOSED (mint + verify refuse) or its capability/session cookies would
 * be forgeable from a source constant. Production without MYPAGE_ACCESS_SECRET is fail-closed too.
 *
...
export function accessSecret(
  ...
  return env.MYPAGE_ACCESS_SECRET ?? (devAuthEnabled(env) ? "test_mypage_access_dev" : undefined);
}
